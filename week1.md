...
- 다들 커뮤니티 성장에 집중하다보니까, Random 채널에 너무 많은 글들이 올라왔다. 글이 많아서 스크롤의 압박과 하나씩 찾아보는게 어려웠다. 다음부터는 Random 채널에 올리기를 지양하거나, 20자 이내의 요약을 하고서 올리는게, 사람들이 읽기 좋을 것 같다.

## 해당 퀘스트는 일주일동안 수행하기에 난이도나 분량이 어땠나요?
- 할만한 사람도 있었고, 매우 작게 설정했음에도 불구하고 제대로 하지 못한 사람도 있었다. 그렇다고 난이도를 너무 낮추면 재미가 없을거라 생각되기도 한다. 
- 과제를 수행하는데 있어서 수행시간대를 지정하면 더 도움이 될 것이다.
- 과제를 끝내고 퀘스트를 수행하는 것은 불가능하다. 차라리 과제 중간에 퀘스트를 수행하도록 유도하는게 좋을 것 같다.

## 이 외에도 한 주간 AI를 활용한 경험을 자유롭게 나눠 보세요. 
- 학습하는데 있어서 AI가 정확하고 필요한 정보를 알려주지 않는다. 정보의 정확성을 높히기 위해선, 정확한 정보가 있는 공식 문서를 찾는게 좋다. 하지만 AI를 요약이 아닌 출처 찾기에만 사용하면, 뭔가 반만 사용하는 것 같다. 
- AI의 정보는 너무 얻기 쉽고, 이쁘게 정리되어 있다. 그만큼 정보의 중요성에 대해 무감각해지는 거 같아, 정보의 휘발성이 강해진다고 생각이 들었다.
